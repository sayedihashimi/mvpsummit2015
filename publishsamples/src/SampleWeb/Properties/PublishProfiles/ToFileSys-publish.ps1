...
"Script directory [$scriptDir]" | Write-Output

$extraFilesPath = (Join-Path $scriptDir '..\..\..\..\media')
if(Test-Path $extraFilesPath){
	$destDir = (Join-Path $packOutput 'wwwroot\media')
	if(-not (Test-Path $destDir)){
		New-Item -ItemType Directory -Path $destDir
	}

	Get-ChildItem -Path $extraFilesPath | Copy-Item -Destination $destDir
}
else{
    "Did not find folder at [$extraFilesPath]" | Write-Output
}
