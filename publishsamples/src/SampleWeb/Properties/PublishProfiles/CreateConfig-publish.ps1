...
function Get-ConfigContentForEnv{
    [cmdletbinding()]
    param(
        [Parameter(Position=0,Mandatory=$true)]
        [System.IO.DirectoryInfo]$destFolder,
        [string]$envName = 'production',
        [string]$configUrlRoot = 'https://gist.githubusercontent.com/sayedihashimi/6607cd7b81d5f85d78ef/raw/bd588ffcf795c65413771465aabc0d8b164fbb85/'
    )
    process{
        if(-not (Test-Path $destFolder)){
            throw "webroot not found at $destFolder"
        }

        [System.IO.FileInfo]$destFile = (Join-Path $destFolder "appsettings.$envName.json")

        $configUrl = ('{0}config.{1}.json' -f $configUrlRoot,$envName)
        
        # download the file to the destination
        (new-object Net.WebClient).DownloadFile($configUrl,$destFile.FullName);
    }
}
...
$envname = $publishProperties['TargetEnvName']

Get-ConfigContentForEnv -destFolder (Join-Path $packOutput 'approot\src\SampleWeb') -envName $envname
