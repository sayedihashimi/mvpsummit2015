...
'secret: [{0}]' -f ((Get-AzureKeyVaultSecret -VaultName SayedKeyVault -Name 'Data-DefaultConnection-ConnectionString').SecretValueText) | Write-Output
...
function New-ConfigFileForEnvironmentFromKeyVault{
    [cmdletbinding()]
    param(
        [string]$keyVaultUri = $env:keyVaultUri,
        [string]$clientId = $env:clientId,
        [string]$clientSecret = $env:clientSecret
    )
    process{
        if([string]::IsNullOrWhiteSpace($keyVaultUri)){ throw ('Missing required env var $env:keyVaultUri')}
        if([string]::IsNullOrWhiteSpace($clientId)){ throw ('Missing required env var $env:clientId')}
        if([string]::IsNullOrWhiteSpace($clientSecret)){ throw ('Missing required env var $env:clientSecret')}
